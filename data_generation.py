...
def generate_data_samples(rng):
    """Generate data samples based on data_sampling_mode configuration"""
...
def compute_UV_unweighted(X0, W_x, W_tau, b, ts, w, rng):
    """Compute U and V matrices using configurable data sampling mode"""
    ...
    for i, (t, wt) in enumerate(zip(ts, w)):
        ...
        if data_sampling_mode == "centers":
            ...
            for batch_idx in range(n_batches):
                ...
                end_idx = min((batch_idx + 1) * batch_size, n_data)
                ...
                # mean term for this batch
                mean_i_batch = WX_X0T_batch * (at / math.sqrt(d + ridge))  # (p, batch_size_actual)
                # preactivations for this batch
                if bias_switch:
                    Z_all_batch = mean_i_batch[:, :, None] + WZ_over_sqrt_d_batch + b[:, None, None] + Wphi[:, None, None]  # (p, batch_size_actual, m_z)
                else:
                    Z_all_batch = mean_i_batch[:, :, None] + WZ_over_sqrt_d_batch + Wphi[:, None, None]  # (p, batch_size_actual, m_z)
                ...
                H_mean_batch = H_batch.mean(axis=2)  # (p, batch_size_actual)
                V_t_hat += (at / (n_data * p)) * (X0_batch.T @ H_mean_batch.T)  # (d, p)
