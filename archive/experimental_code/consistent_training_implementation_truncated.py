...
def train_simpson_consistent(
    ...
    epochs=200000, lr=0.01, T=10.0, eps=1e-5, device='cuda', A_ref=None
): 
...
def run_consistent_experiment():
    ...
    loss_history, trained_model, A_diff_history = train_simpson_consistent(
            model, U_tilde_np=U_truncated, V_tilde_np=V_tilde, epochs=200000, lr=0.01, T=T,A_ref=A_ref_tensor  # Reduced for faster comparison
        )
