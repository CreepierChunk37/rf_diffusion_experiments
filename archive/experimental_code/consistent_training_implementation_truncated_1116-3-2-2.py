...
def compute_UV_unweighted_consistent(X0, W_x, W_tau, b, ts, w, rng, p, d, n, K_t, T, 
    ...
    for i, (t, wt) in tqdm(enumerate(zip(ts, simpson_weights))):
        ...
        Z_batch = rng.standard_normal((n_data, z_grid_size, d))
        ...
        WZ_over_sqrt_d = np.einsum('pd,nmd->pnm', W_x, sqrt_ht_Z_batch)    # (p, z_grid_size)
        ...
        if bias_switch:
            Z_all_batch = mean_i[:, :, None] + WZ_over_sqrt_d + b[:, None, None] + Wphi[:, None, None]  # (p, n_data, m_batch_size)
        else:
            Z_all_batch = mean_i[:, :, None] + WZ_over_sqrt_d + Wphi[:, None, None]  # (p, n_data, m_batch_size)

        H = np.maximum(0.0, Z_all_batch)  # (p, n_data, m_batch_size)

        H_flat = H.reshape(p, -1)  # (p, n_data * m_batch_size)
...
def sample_exponential(model, num_samples=1000, dim=2, num_steps=4000, T=40.0, device='cuda',
    ...
    t_min = 5e-3  # Simulation start time (epsilon)
...
def compare_different_feature_dims():
    ...
    for feature_dim in feature_dims:
        ...
        loss_history, trained_model, A_diff_history = train_simpson_consistent(model, U_tilde_np=U_tilde, V_tilde_np=V_tilde, C = -ground_truth, epochs=6000, lr=0.001, T=T,A_ref=A_ref_tensor)
        ...
        samples = sample_exponential(
            model_1, num_samples=2000, T=T, device=device
        )
