...
def compare_different_feature_dims():
    ...
    K_t = 128
    T = 40.0
    L_t = 2000 # Number of time points for integration
    ...
    for feature_dim in feature_dims:
        ...
        ground_truth = term1 - term2 + C
        print(ground_truth)

        loss_history, trained_model, A_diff_history = train_simpson_consistent(
            model, U_tilde_np=U_tilde, V_tilde_np=V_tilde, C= - term1 + term2 , epochs=100000, lr=0.05, T=T, A_ref=A_ref_tensor)

        fig, axs = plt.subplots(2, 2, figsize=(12, 10))
        fig.suptitle('Training History Visualization', fontsize=16)
        
        # 1. loss_history 正常坐标图
        axs[0, 0].plot(loss_history)
        axs[0, 0].set_title('Loss History (Normal Scale)')
        axs[0, 0].set_xlabel('Epochs')
        axs[0, 0].set_ylabel('Loss')
        axs[0, 0].grid(True)
        
        # 2. loss_history 对数坐标图
        axs[0, 1].semilogy(loss_history)
        axs[0, 1].set_title('Loss History (Log Scale)')
        axs[0, 1].set_xlabel('Epochs')
        axs[0, 1].set_ylabel('Loss (log)')
        axs[0, 1].grid(True)
        
        # 3. A_diff_history 正常坐标图
        axs[1, 0].plot(A_diff_history)
        axs[1, 0].set_title('A Difference History (Normal Scale)')
        axs[1, 0].set_xlabel('Epochs')
        axs[1, 0].set_ylabel('A Difference')
        axs[1, 0].grid(True)
        
        # 4. A_diff_history 对数坐标图
        axs[1, 1].semilogy(A_diff_history)
        axs[1, 1].set_title('A Difference History (Log Scale)')
        axs[1, 1].set_xlabel('Epochs')
        axs[1, 1].set_ylabel('A Difference (log)')
        axs[1, 1].grid(True)
        ...
        print("Last 10 loss values:", loss_history[-10:])
        print("Last 10 A difference values:", A_diff_history[-10:])
