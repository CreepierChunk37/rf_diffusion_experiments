...
def compute_A_infty(U_tilde_np, V_tilde_np, p):
    U_tilde_f32 = U_tilde_np.astype(np.float32)
    V_tilde_f32 = V_tilde_np.astype(np.float32)

    try:
        U_sym = 0.5 * (U_tilde_f32 + U_tilde_f32.T)
        eigvals, Q = np.linalg.eigh(U_sym)
        tol = np.finfo(np.float32).eps * max(eigvals.shape) * np.max(np.abs(eigvals))

        lam = np.clip(eigvals, 0.0, None)
        Qt = Q.T
        invlam = np.zeros_like(lam)
        mask = lam > max(tol, 1e-7) 
