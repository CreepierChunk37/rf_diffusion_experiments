...
def compute_UV_unweighted(X0, W_x, W_tau, b, ts, w, rng, p, d, T, K, bias_switch=1):
    ...
    for i, (t, wt) in enumerate(zip(ts, w)):
        ...
        U_acc += wt * U_t_hat / ht_square
        V_acc += wt * V_t_hat / ht_square
...
def compute_constant_term_consistent(data_np, ts, w, T):
    ...
    # 1. 计算被积函数的核心部分
    # 使用与 compute_UV_unweighted 一致的定义
    at_values = np.exp(-ts)
    ht_values = 1.0 - np.exp(-2.0 * ts)
    
    # 加上微小量防止除0（虽然你的ts从1e-3开始，但保持稳健）
    ht_square = ht_values ** 2 + 1e-16
    
    # 2. 计算积分项: sum( w * (a_t^2 / h_t^2) )
...
def compare_different_feature_dims():
    ...
    for feature_dim in feature_dims:
        ...
        C = compute_constant_term_consistent(data_np, ts, w, T)
        ...
        loss_history, trained_model, A_diff_history = train_simpson_consistent(model, U_tilde_np=U_tilde, V_tilde_np=V_tilde, C = -ground_truth, epochs=600, lr=0.001, T=T,A_ref=A_ref_tensor)
        
        final_loss = 0.0
        model_1 = copy.deepcopy(model)
        with torch.no_grad():
            model_1.A = nn.Parameter(A_ref_tensor.clone().to(device))
        ...
        snapshots_data = sample_exponential(model=model_1, num_samples=2000, device=device, num_snapshots=20)
