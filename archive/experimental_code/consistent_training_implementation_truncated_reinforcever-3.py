...
def visualize_results(samples, centers, counts, feature_dim, tau, loss_val, save_dir="results_randominit"):
    ...
    try:
        sns.kdeplot(x=samples[:, 0], y=samples[:, 1], 
                    cmap="Oranges", fill=True, levels=20, alpha=0.8, thresh=0.05)
    except Exception as e:
        print(f"Warning: KDE plot failed ({e}), skipping density background.")

    # 2. 绘制生成的样本点 (蓝色小点)
    plt.scatter(samples[:, 0], samples[:, 1], s=2, c='blue', alpha=0.6, label='Generated')

    # 3. 绘制真实数据中心 (红色 X)
    plt.scatter(centers[:, 0], centers[:, 1], s=150, c='red', marker='x', linewidth=2, label='True Centers')

    # 4. 在数据中心附近标注数量 (n=xxx)
    for i, center in enumerate(centers):
        count = counts[i]
        bbox_props = dict(boxstyle="round,pad=0.3", fc="white", ec="brown", alpha=0.9)
        plt.text(center[0] + 0.3, center[1] + 0.3, f"n={count}", 
                 fontsize=12, fontweight='bold', color='darkred', bbox=bbox_props)

    plt.title(f"Dim={feature_dim}, Tau={tau_str}, Loss={loss_val:.4f}", fontsize=14)
    plt.xlim(-2, 7)
    plt.ylim(-2, 7)
    ...
    save_path = os.path.join(save_dir, f"dim_{feature_dim}_tau_{file_suffix}.png")
    ...
    for p in feature_dims:
        ...
        for tau in taus:
            if tau == float('inf'):
                ...
                # 计算 A(tau) / sqrt(p)
                W_tau = calculate_A_tau(tau, U_tilde, V_tilde, A_init)
...
if __name__ == "__main__":
    DIMS_TO_TEST = [500, 1000, 2000, 4000]
    TAUS_TO_TEST = [5, 10, 50, 100, 200, 500, 1000, 5000, 10000, float('inf')] 
