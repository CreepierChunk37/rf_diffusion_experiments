...
def sample_exponential(model, num_samples=1000, dim=2, num_steps=10000, T=40.0, device='cuda',
    ...
    t_min = 0.001  # Simulation start time (epsilon)
...
def run_consistent_experiment():
    ...
    loss_history, trained_model, A_diff_history = train_simpson_consistent(
            model, U_tilde_np=U_tilde, V_tilde_np=V_tilde, epochs=1000000, lr=0.05, T=T,A_ref=A_ref_tensor  # Reduced for faster comparison
        )
...
def compare_different_feature_dims():
    ...
    # Model parameters
    input_dim = 2
    feature_dims = [128,512,2048,4096]  # Different feature dimensions to compare
    K_t = 128
    T = 40.0
    L_t = 1000  # Number of time points for integration
    ...
    for feature_dim in feature_dims:
        ...
        loss_history, trained_model, A_diff_history = train_simpson_consistent(
            model, U_tilde_np=U_tilde, V_tilde_np=V_tilde, C=C, epochs=1000000, lr=0.05, T=T, A_ref=A_ref_tensor)
    ...
    # Create individual plots for each feature_dim
    ...
    print("\n" + "="*80)
    print("FEATURE_DIM COMPARISON SUMMARY")
    print("="*80)
    for feature_dim, results in results_dict.items():
        print(f"\nfeature_dim={feature_dim}:")
        print(f"  Final Loss: {results['final_loss']:.6f}")
        print(f"  Final A Difference: {results['final_A_diff']:.6f}")
        if results['A_diff_history']:
            print(f"  Initial A Difference: {results['A_diff_history'][0]:.6f}")
            improvement = (results['A_diff_history'][0] - results['A_diff_history'][-1]) / results['A_diff_history'][0] * 100
            print(f"  Improvement: {improvement:.2f}%")
    print("="*80)
    
    # Create comparison plots
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Comparison Across Different Feature Dimensions', fontsize=16, fontweight='bold')
    
    colors = ['blue', 'red', 'green', 'orange']
    
    # 1. Loss History Comparison
    for i, (feature_dim, results) in enumerate(results_dict.items()):
        epochs = list(range(1, len(results['loss_history']) + 1))
        axes[0, 0].plot(epochs, results['loss_history'], 
                       color=colors[i], linewidth=2, label=f'feature_dim={feature_dim}')
    axes[0, 0].set_xlabel('Epoch', fontsize=12)
    axes[0, 0].set_ylabel('Loss', fontsize=12)
    ...
    for i, (feature_dim, results) in enumerate(results_dict.items()):
        ...
        axes[1, 0].scatter(samples[:, 0], samples[:, 1], alpha=0.5, s=15, 
                          color=colors[i], label=f'feature_dim={feature_dim}')
    ...
    for i, (feature_dim, results) in enumerate(results_dict.items()):
        ...
        try:
            sns.kdeplot(x=samples[:, 0], y=samples[:, 1], 
                       ...
                       label=f'feature_dim={feature_dim}')
        except ValueError:
            pass
    ...
    plt.tight_layout()
    plt.savefig('feature_dim_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
