...
def compute_UV_unweighted_consistent(X0, W_x, W_tau, b, ts, w, rng, p, d, n, K_t, T, 
                                    z_grid_size=700, ridge=1e-9, bias_switch=1):
    """
    ...
    simpson_weights /= (3.0 * (t_grid_size - 1))  # Normalize by 3*(n-1)
    
    # Generate fixed z_grid ONCE (same as train_simpson approach)
    # This is the key fix - use same z_grid for all time points
    z_grid_np = rng.standard_normal((z_grid_size, d))  # (z_grid_size, d)
    
    for i, (t, wt) in enumerate(zip(ts, simpson_weights)):
        ...
        WZ_over_sqrt_d = np.einsum('pd,md->pm', W_x, z_grid_np) / math.sqrt(d)  # (p, z_grid_size)
...
def train_simpson_consistent(model, data, z_grid, epochs=20000, lr=1.0, T=10.0, eps=1e-5, device='cuda',
          t_grid_size=201, z_grid_size=700, t_batch_size=70, A_ref=None): 
    """
    Consistent implementation of train_simpson - unchanged as it already uses the correct methods
    """
...
def run_consistent_experiment():
    ...
    K_t = 64
    T = 40.0
    L_t = 200  # Number of time points for integration
    ...
    U_tilde, V_tilde = compute_UV_unweighted_consistent(
        numpy_array, W_x_np, W_t_np, b_np, ts, w, rng, p, d, n, K_t, T
    )
    
    A_ref = compute_A_infty(U_tilde, V_tilde, p)
    A_ref_tensor = torch.tensor(A_ref, device=device, dtype=torch.float32)
    
    # Train model using consistent method
    print("Training model with consistent implementation...")
    loss_history, trained_model, A_diff_history = train_simpson_consistent(
        model, data, z_grid, lr=1.0, T=T, device=device, 
        t_grid_size=201, A_ref=A_ref_tensor
    )
    
    # Evaluate final model performance
    final_loss = evaluate_model_loss_consistent(
        trained_model, data, z_grid, T=T, device=device, t_grid_size=201
    )
...
def compare_different_t_grid_sizes():
    ...
    for t_grid_size in t_grid_sizes:
        ...
        z_grid = torch.randn(700, *data.shape[1:], device=device)
        loss_history, trained_model, A_diff_history = train_simpson_consistent(
            model, data, z_grid, lr=0.5, T=T, device=device, 
            t_grid_size=t_grid_size, A_ref=A_ref_tensor, epochs=20000  # Reduced for faster comparison
        )
